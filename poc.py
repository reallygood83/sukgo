...
class Tool:
    ...
    asks_questions: bool = False  # 본 분석 전 LLM 이 컨텍스트 질문 단계 실행
...
# ═════════════════════════════════════════════════════════════════════
# Clarifying Questions — 본 분석 전 컨텍스트 수집
# ═════════════════════════════════════════════════════════════════════
CLARIFY_PROMPT = """당신은 시니어 컨설턴트입니다. **{tool_name}** ({tool_desc}) 분석을 시작하기 전에, 사용자에게 가장 중요한 컨텍스트를 묻는 단계입니다.

**사용자가 입력한 주제**:
{topic}

이 주제로 깊이 있는 맞춤형 분석을 하기 위해 **반드시 알아야 할 핵심 질문 3~5개만** 생성하세요.

원칙:
1. **답이 분석을 크게 바꿀 질문만**: 일반론과 맞춤형 분석의 차이를 만드는 질문
2. **사용자가 1분 안에 답할 수 있는 구체적 질문** (장황한 서술 X)
3. **각 질문에 짧은 예시** 첨부해서 사용자가 답하는 방식 가이드
4. 너무 사적이거나 답하기 어려운 질문은 피하기 (사용자가 건너뛸 수 있도록 안내)
5. 질문 외 다른 설명·인사·결론 절대 추가하지 말 것

**출력 형식** (정확히 이대로, 마크다운 그대로):
...
마지막에 한 줄 추가: `_답하기 어려운 질문은 'pass' 라고 적거나 빈칸으로 두세요._`
"""
...
def read_multi_line(end_marker: str = "---") -> str:
    """여러 줄 입력 받기. '---' 줄 또는 빈 줄 두 번 연속 입력 시 종료."""
    print(f"  {DIM}(전부 입력 후 빈 줄에서 Enter 두 번, 또는 '{end_marker}' 입력){RESET}")
    print(f"  {DIM}(전체를 건너뛰려면 첫 줄에 'skip' 입력){RESET}")
    lines = []
    blank_streak = 0
    while True:
        try:
...
def _clarify_quick(tool: Tool, topic: str, questioner: "Backend") -> str:
    ...
    questions_prompt = CLARIFY_PROMPT.format(
        tool_name=tool.name,
        tool_desc=tool.short_desc,
        topic=topic,
    )
    questions = questioner.chat(questions_prompt)

    if not questions or len(questions.strip()) < 30:
        print(f"  {YELLOW}⚠  질문 생성 실패 — 컨텍스트 수집 건너뜁니다.{RESET}")
        return topic
    ...
    print(f"  {BOLD}답변:{RESET}")
    answers = read_multi_line()

    if not answers:
        print(f"\n  {DIM}컨텍스트 없이 진행합니다.{RESET}")
        return topic
...
def tool_flow(config: dict, tool: Tool):
    ...
    # 도메인 컨설턴트(Career/Education 등)는 컨텍스트 수집 단계 먼저
    if tool.asks_questions:
        topic = clarify_phase(tool, topic, selected[0])

    prompt = tool.get_prompt().format(topic=topic)

    if tool.asks_questions:
        print(f"\n  {BOLD}{TEAL}━━━ 2단계: 본 분석 ━━━{RESET}\n")
...
def investment_flow(config: dict, tool: Tool):
    ...
    # ── 컨텍스트 수집 (투자 호라이즌·리스크 허용도·현재 포지션 등) ──
    if tool.asks_questions:
        user_topic = clarify_phase(tool, user_topic, selected[0])

    # ── 프롬프트 조립 ──
    prompt = tool.get_prompt().format(topic=user_topic, stock_data=data_summary)

    print(f"\n  {BOLD}{TEAL}━━━ 본 분석 단계 ━━━{RESET}\n")
    ...
    if mode == "synthesis":
        ...
        print(f"  {TEAL}⠋  9섹션 종합 리포트 생성 중... (백엔드별 30~120초){RESET}")
        responses = run_compare(prompt, selected, render_inline=(mode == "compare"))
        if mode == "single" and responses:
            render_response(list(responses.values())[0])
